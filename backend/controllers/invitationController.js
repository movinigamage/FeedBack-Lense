...
exports.sendInvitations = async (req, res) => {
  try {
    ...
    if (error.code === 'SURVEY_INACTIVE') {
      return res.status(400).json({ error: 'Cannot send invitations to inactive survey' });
    }
    if (error.code === 'MISSING_USER_EMAILS') {
      return res.status(400).json({ error: 'User emails array is required' });
    }
...
exports.getReceivedInvitations = async (req, res) => {
  try {
    ...
    const invitations = await invitationService.getUserReceivedInvitations(userId);
